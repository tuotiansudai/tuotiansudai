...
REDIS_DB = config.get_int('common.token.db', default_value=0)

MOBILE_TOKEN_EXPIRED_SECONDS = config.get_int('mobile.app.token.expire.seconds', default_value=2592000)  # 30 days
WEB_TOKEN_EXPIRED_SECONDS = config.get_int('web.login.lock.seconds', default_value=1800)  # 30 minutes
LOGIN_FAILED_MAXIMAL_TIMES = config.get_int('mobile.login.max.failed.times', default_value=3)
LOGIN_FAILED_EXPIRED_SECONDS = config.get_int('mobile.login.lock.seconds', default_value=1800)  # 30 minutes

ALIYUN_MNS_ENABLED = config.get_bool('aliyun.mns.enabled')
...
MQ_REDIS_DB = config.get_int('common.redis.db', default_value=2)

MYSQL_HOST = config.get('common.jdbc.host', default_value='192.168.33.10')
MYSQL_PORT = config.get_int('common.jdbc.port', default_value=3306)
