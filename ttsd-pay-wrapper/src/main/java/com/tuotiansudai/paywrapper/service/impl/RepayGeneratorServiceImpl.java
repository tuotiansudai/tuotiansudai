...
public class RepayGeneratorServiceImpl implements RepayGeneratorService {
    ...
    public void generateRepay(long loanId) throws PayException {
        ...
        logger.debug(MessageFormat.format("[Generate_Repay:] loanId:{0} loanRepay generate begin",loanId));
        for (int index = 0; index < totalPeriods; index++) {
            logger.debug(MessageFormat.format("[Generate_Repay:] loanRepay generate loanId:{0} period:{1} begin",loanId,index + 1));
            int period = index + 1;
            ...
            for (InvestModel successInvestModel : successInvestModels) {
                logger.debug(MessageFormat.format("[Generate_Repay:]investRepay generate loanId:{0},investId:{1} period:{2}  end",loanId,successInvestModel.getId(),period));
            ...
            logger.debug(MessageFormat.format("[Generate_Repay:] loanId:{0} loanRepay generate {1} period end",loanId,index + 1));
        }
        logger.debug(MessageFormat.format("[Generate_Repay:] loanId:{0} loanRepayModels size:{1} investRepayModels size:{2} begin",
                loanId,loanRepayModels == null?0:loanRepayModels.size(),investRepayModels == null?0:investRepayModels.size()));
