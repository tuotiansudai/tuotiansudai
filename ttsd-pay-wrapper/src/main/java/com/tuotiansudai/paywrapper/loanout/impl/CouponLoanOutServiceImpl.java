...
public class CouponLoanOutServiceImpl implements CouponLoanOutService {
    ...
    public String transferRedEnvelopNotify(Map<String, String> paramsMap, String queryString) {
        ...
        if (callbackRequest == null || Strings.isNullOrEmpty(callbackRequest.getOrderId()) || !callbackRequest.getOrderId().contains("X")) {
            logger.error(MessageFormat.format("[标的放款:出借红包] transfer red envelop payback callback parse is failed (queryString = {0})", queryString));
            return null;
        }
        ...
        if (userCouponModel == null) {
            logger.error(MessageFormat.format("[标的放款:出借红包] TransferRedEnvelopCallback payback callback failed, order id({0}) is not exist", callbackRequest.getOrderId()));
            return callbackRequest.getResponseData();
        }
        ...
        logger.info(MessageFormat.format("[标的放款:出借红包] send message TransferRedEnvelopCallback,loanId:{0}, investId:{1}, loginName:{2} queryString:{3}, orderId:{4}",
    ...
    public boolean sendRedEnvelopTransferInBalanceCallBack(long userCouponId) {
        logger.info(MessageFormat.format("[标的放款:出借红包] send redEnvelop transfer in balance callBack, userCouponId:{0}", String.valueOf(userCouponId)));
        UserCouponModel userCouponModel = userCouponMapper.findById(userCouponId);
        if (userCouponModel.getActualInterest() != 0) {
            return true;
        }
        ...
        try {
            ...
            logger.error(MessageFormat.format("[标的放款:出借红包] red envelope coupon transfer in balance failed (userCouponId = {0})", String.valueOf(userCouponModel.getId())), e);
        }
