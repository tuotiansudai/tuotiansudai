CREATE TABLE `ump_operations`.`hz_nopwd_repay_request` (
...
CREATE TABLE `ump_operations`.`hz_nopwd_repay_response` (
  `id`             BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  ...
  PRIMARY KEY (`id`),
  CONSTRAINT FK_HZ_NOPWD_REPAY_RESPONSE_REQUEST_ID_REF_REQUEST_ID FOREIGN KEY (`request_id`) REFERENCES `ump_operations`.`hz_nopwd_repay_request` (`id`)
)
  ENGINE = InnoDB
  AUTO_INCREMENT = 100001
  DEFAULT CHARSET = utf8;
