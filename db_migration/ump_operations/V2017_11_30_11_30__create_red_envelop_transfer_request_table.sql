CREATE TABLE `ump_operations`.`red_envelop_transfer_request` (
...
CREATE TABLE `ump_operations`.`red_envelop_transfer_response` (
  `id`             BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  ...
  PRIMARY KEY (`id`),
  CONSTRAINT FK_RED_ENVELOP_RESPONSE_REF_RED_ENVELOP_ID FOREIGN KEY (`request_id`) REFERENCES `ump_operations`.`red_envelop_transfer_request` (`id`)
)
  ENGINE = InnoDB
  AUTO_INCREMENT = 100001
  DEFAULT CHARSET = utf8;

CREATE TABLE `ump_operations`.`red_envelop_transfer_notify_request` (
  `id`             BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `service`        VARCHAR(64),
  `sign_type`      VARCHAR(16),
