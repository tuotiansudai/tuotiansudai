CREATE TABLE `ump_operations`.`credit_loan_transfer_agent_request` (
...
CREATE TABLE `ump_operations`.`credit_loan_transfer_agent_response` (
  `id`             BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  ...
  PRIMARY KEY (`id`),
  CONSTRAINT FK_CREDIT_LOAN_TRANSFER_AGENT_REF_REQUEST_ID FOREIGN KEY (`request_id`) REFERENCES `ump_operations`.`credit_loan_transfer_agent_request` (`id`)
)
  ENGINE = InnoDB
  AUTO_INCREMENT = 100001
  DEFAULT CHARSET = utf8;
