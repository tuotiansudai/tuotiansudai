CREATE TABLE `ump_operations`.`current_deposit_request` (
  ...
  PRIMARY KEY (`id`),
  KEY `IDX_CURRENT_DEPOSIT_REQUEST_ORDER_ID` (`order_id`)
)
  ENGINE = InnoDB
  AUTO_INCREMENT = 100001
  DEFAULT CHARSET = utf8;

CREATE TABLE `ump_operations`.`current_deposit_response` (
  `id`             BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  ...
  PRIMARY KEY (`id`),
  CONSTRAINT FK_CURRENT_DEPOSIT_RESPONSE_ID_REF_REQUEST_ID FOREIGN KEY (`request_id`) REFERENCES `ump_operations`.`current_deposit_request` (`id`)
)
  ENGINE = InnoDB
  AUTO_INCREMENT = 100001
  DEFAULT CHARSET = utf8;