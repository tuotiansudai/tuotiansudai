CREATE TABLE `ump_operations`.`credit_loan_out_project_transfer_request` (
...
CREATE TABLE `ump_operations`.`credit_loan_out_project_transfer_notify_request` (
  `id`             BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
...
CREATE TABLE `ump_operations`.`credit_loan_out_project_transfer_response` (
  `id`             BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  ...
  PRIMARY KEY (`id`),
  CONSTRAINT FK_CREDIT_LOAN_OUT_PROJECT_TRANSFER_RESPONSE_REQUEST_ID FOREIGN KEY (`request_id`) REFERENCES `ump_operations`.`credit_loan_out_project_transfer_request` (`id`)
)
  ENGINE = InnoDB
  AUTO_INCREMENT = 100001
  DEFAULT CHARSET = utf8;
