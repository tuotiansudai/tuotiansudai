CREATE TABLE `ump_operations`.`credit_loan_recharge_nopwd_request` (
...
CREATE TABLE `ump_operations`.`credit_loan_recharge_nopwd_response` (
  ...
  PRIMARY KEY (`id`),
  CONSTRAINT FK_CREDIT_LOAN_NOPWD_RECHARGE_REF_REQUEST_ID FOREIGN KEY (`request_id`) REFERENCES `ump_operations`.`credit_loan_recharge_nopwd_request` (`id`)
)
  ENGINE = InnoDB
  AUTO_INCREMENT = 100001
  DEFAULT CHARSET = utf8;
