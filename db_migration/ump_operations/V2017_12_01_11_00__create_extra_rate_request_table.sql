CREATE TABLE `ump_operations`.`extra_rate_transfer_request` (
...
CREATE TABLE `ump_operations`.`extra_rate_transfer_response` (
  `id`             BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  ...
  PRIMARY KEY (`id`),
  CONSTRAINT FK_EXTRA_RATE_RESPONSE_REF_EXTRA_RATE_ID FOREIGN KEY (`request_id`) REFERENCES `ump_operations`.`extra_rate_transfer_request` (`id`)
)
  ENGINE = InnoDB
  AUTO_INCREMENT = 100001
  DEFAULT CHARSET = utf8;

ALTER TABLE `ump_operations`.`extra_rate_notify_request` DROP `status`;