CREATE TABLE `ump_operations`.`hz_activate_account_request` (
...
CREATE TABLE `ump_operations`.`hz_activate_account_notify_request` (
  `id`             BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
...
CREATE TABLE `ump_operations`.`hz_activate_account_response` (
  `id`             BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  ...
  PRIMARY KEY (`id`),
  CONSTRAINT FK_HZ_ACTIVATE_ACCOUNT_RESPONSE_REQUEST_ID_REF_REQUEST_ID FOREIGN KEY (`request_id`) REFERENCES `ump_operations`.`hz_activate_account_request` (`id`)
)
  ENGINE = InnoDB
  AUTO_INCREMENT = 100001
  DEFAULT CHARSET = utf8;