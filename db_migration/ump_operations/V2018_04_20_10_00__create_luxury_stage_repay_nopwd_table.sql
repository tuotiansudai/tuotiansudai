CREATE TABLE `ump_operations`.`luxury_stage_repay_nopwd_request` (
...
CREATE TABLE `ump_operations`.`luxury_stage_repay_nopwd_response` (
  `id`             BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  ...
  PRIMARY KEY (`id`),
  CONSTRAINT FK_LUXURY_STAGE_REPAY_NOPWD_RESPONSE_REQUEST_ID_REF_REQUEST_ID FOREIGN KEY (`request_id`) REFERENCES `ump_operations`.`luxury_stage_repay_nopwd_request` (`id`)
)
  ENGINE = InnoDB
  AUTO_INCREMENT = 100001
  DEFAULT CHARSET = utf8;