CREATE TABLE `ump_operations`.`payroll_transfer_request` (
...
CREATE TABLE `ump_operations`.`payroll_transfer_response` (
  `id`             BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  ...
  PRIMARY KEY (`id`),
  CONSTRAINT FK_PAYROLL_TRANSFER_RESPONSE_REF_REQUEST_ID FOREIGN KEY (`request_id`) REFERENCES `ump_operations`.`payroll_transfer_request` (`id`)
)
  ENGINE = InnoDB
  AUTO_INCREMENT = 100001
  DEFAULT CHARSET = utf8;