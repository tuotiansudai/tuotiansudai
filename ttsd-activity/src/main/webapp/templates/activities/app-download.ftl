<#import "../macro/global.ftl" as global>
<@global.main pageCss="${css.company_activity}" pageJavascript="${js.download_app}" activeNav="" activeLeftNav="" title="拓天速贷手机客户端_理财客户端_拓天速贷" keywords="拓天速贷,APP理财,移动客户端,网络理财,P2P理财,拓天速贷APP" description="拓天速贷手机客户端为理财,投资,贷款等投融资用户提供安全、专业、便捷的互联网金融理财服务。">

<div class="download-container">
    <div class="wp-img">
        <img src="${staticServer}/activity/images/sign/actor/download/download-top-new.png" alt="" width="100%">
        <a href="https://itunes.apple.com/cn/app/ta-tian-su-dai/id1039233966?mt=8" class="ios" target="_blank"></a>
        <a href="${staticServer}/app/tuotiansudai.apk" class="android"></a>
    </div>
    <div class="download-one">
        <div class="step-one">
            <img src="${staticServer}/activity/images/sign/actor/download/download-one-step.png" alt="">
        </div>
    </div>
    <div class="download-two">
        <div class="step-two">
            <img src="${staticServer}/activity/images/sign/actor/download/download-two-step.png" alt="">
        </div>
    </div>
    <div class="download-three">
        <div class="step-three">
            <img src="${staticServer}/activity/images/sign/actor/download/download-three-step.png" alt="">
        </div>
    </div>
    <div class="download-four">
        <div class="step-four">
            <img src="${staticServer}/activity/images/sign/actor/download/download-four-step.png" alt="">
        </div>
    </div>
    <div class="download-five">
        <div class="step-five">
            <img src="${staticServer}/activity/images/sign/actor/download/download-five-step.png" alt="">
        </div>
    </div>
     <div class="download-six">
        <div class="step-six">
            <img src="${staticServer}/activity/images/sign/actor/download/download-six-step.png" alt="">
        </div>
        <div class="now-download">
            <a href="">立即下载</a>
        </div>
    </div>
</div>
<div class="download-container-phone">
    <div class="wp-img">
        <img src="${staticServer}/activity/images/sign/actor/download/download-phone-topnew.png" alt="" width="100%">
        <a href="https://itunes.apple.com/cn/app/ta-tian-su-dai/id1039233966?mt=8" class="ios" target="_blank"></a>
        <a href="${staticServer}/app/tuotiansudai.apk" class="android" target="_blank"></a>
    </div>
    <div class="download-one">
       <img src="${staticServer}/activity/images/sign/actor/download/phone-one.png" alt="" width="100%">
    </div>
    <div class="download-two">
       <img src="${staticServer}/activity/images/sign/actor/download/phone-two.png" alt="" width="100%">
    </div>
    <div class="download-three">
       <img src="${staticServer}/activity/images/sign/actor/download/phone-three.png" alt="" width="100%">
    </div>
    <div class="download-four">
       <img src="${staticServer}/activity/images/sign/actor/download/phone-four.png" alt="" width="100%">
    </div>
    <div class="download-five">
        <img src="${staticServer}/activity/images/sign/actor/download/phone-five.png" alt="" width="100%">
    </div>
     <div class="download-six">
        <img src="${staticServer}/activity/images/sign/actor/download/phone-six.png" alt="" width="100%">
       <a href="javascript:void(0)" id="downloadBtn">立即下载</a>
    </div>
</div>
</@global.main>