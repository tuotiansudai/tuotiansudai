...
public class JobInitPlugin implements SchedulerPlugin {
    ...
    public void start() {
        ...
        if (JobType.LoanBirthdayNotify.name().equalsIgnoreCase(schedulerName)) {
            createLoanBirthdayNotifyJob();
        }
    ...
    private void createAutoJPushAlertBirthMonth() {
        try {
            jobManager.newJob(JobType.AutoJPushAlertBirthMonth, AutoJPushAlertBirthMonthJob.class).replaceExistingJob(true)
                    //TEST
                    .runWithSchedule(CronScheduleBuilder.cronSchedule("0 30 15 * * ? *").inTimeZone(TimeZone.getTimeZone("Asia/Shanghai")))
//                    .runWithSchedule(CronScheduleBuilder.cronSchedule("0 0 12 1 * ? *").inTimeZone(TimeZone.getTimeZone("Asia/Shanghai")))
    ...
    private void createLoanBirthdayNotifyJob() {
        try {
            jobManager.newJob(JobType.LoanBirthdayNotify, LoanBirthdayNotifyJob.class).replaceExistingJob(true)
                    .runWithSchedule(CronScheduleBuilder.cronSchedule("0 0 10 1 * ?").inTimeZone(TimeZone.getTimeZone("Asia/Shanghai")))
                    .withIdentity(JobType.LoanBirthdayNotify.name(), JobType.LoanBirthdayNotify.name()).submit();
        } catch (SchedulerException e) {
            logger.debug(e.getLocalizedMessage(), e);
        }
    }
