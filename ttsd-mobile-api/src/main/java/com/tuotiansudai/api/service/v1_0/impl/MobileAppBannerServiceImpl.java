...
public class MobileAppBannerServiceImpl implements MobileAppBannerService {
    ...
    public BaseResponseDto<BannerResponseDataDto> generateBannerList(String loginName, Source source) {
        ...
        for (BannerModel bannerModel : bannerModelList) {
            pictures.add(new BannerPictureResponseDataDto(bannerModel.getTitle(), MessageFormat.format(BANNER_URL_TEMPLATE, bannerModel.getAppUrl()), bannerModel.getSharedUrl(), bannerModel.getOrder(), bannerServer + bannerModel.getAppImageUrl(), bannerModel.getContent(), false));
        }
