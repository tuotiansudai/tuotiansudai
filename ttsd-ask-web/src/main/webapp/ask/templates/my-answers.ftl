...
<div class="article-content fl" id="myQAnswer">
    <ul class="switch-menu clearfix" id="my-questions-tab">
        <li><a href="${global.applicationContext}/question/my-questions">我的提问</a></li>
        <li class="active"><a href="${global.applicationContext}/answer/my-answers">我的回答</a></li>
    </ul>
    <div class="border-ask-box clearfix">
        <div class="answers-box">
            <#list answers.data.records as answer>
                <dl class="answers-list">
                    <dt><a href="${global.applicationContext}/question/${answer.question.id?string.computer}" target="_blank">${answer.question.question}</a></dt>
                    <dd><a href="${global.applicationContext}/question/${answer.question.id?string.computer}" target="_blank">${answer.question.addition!}</a></dd>
                    <dd><span>${answer.question.mobile}</span>
                        ...
                        <span class="datetime">${answer.question.createdTime?string("yyyy年MM月dd日 HH:mm")}</span>
                                ...
                                <span class="fr tag">
                                    <a class="${global.applicationContext}/question/category/${tag.name()}" href="">${tag.description}</a>
                               </span>
                                </#list>
    ...
    <div class="pagination">
        <#if answers.data.hasPreviousPage>
            <a href="${global.applicationContext}/answer/my-answers">首页</a>
        </#if>
        <#if answers.data.index &gt; 3>
            <a href="${global.applicationContext}/answer/my-answers?index=${answers.data.index-1}"> < </a>
        </#if>
        ...
        <#list lower..upper as page>
            <a href="${global.applicationContext}/answer/my-answers?index=${page}" <#if page == answers.data.index>class="active"</#if>> ${page} </a>
        </#list>
        ...
        <#if answers.data.hasNextPage>
            <a href="${global.applicationContext}/answer/my-answers?index=${answers.data.maxPage}">末页</a>
        </#if>
    </div>
</div>
