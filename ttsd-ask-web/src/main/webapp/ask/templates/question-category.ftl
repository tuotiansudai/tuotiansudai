...
<#assign tkdMap = {"SECURITIES":"{'title':'证券问答_拓天速贷','keywords':'证券问答,网贷问答,证券知识,金融问答','descritpion':'拓天速贷投资问答系统,为您解答证券最新最快的投资知识,让您放心投资、安全投资,拓天速贷为投资人答疑解惑.'}",
                   ...
                   "INVEST":"{'title':'理财问答_拓天速贷','keywords':'理财问答,网贷问答,投资知识,金融问答','descritpion':'拓天速贷投资问答系统,为您解答金融行业最新最快的投资知识,让您放心投资、安全投资,拓天速贷为投资人答疑解惑.'}",
...
<div class="article-content fl">
    ...
    <div class="pagination">
        <#if questions.data.hasPreviousPage>
            <a href="${applicationContext}/question/category/${tag.name()}">首页</a>
        </#if>
        <#if questions.data.index &gt; 3>
            <a href="${applicationContext}/question/category/${tag.name()}?index=${questions.data.index-1}"> < </a>
        </#if>
        ...
        <#list lower..upper as page>
            <a href="${applicationContext}/question/category/${tag.name()}?index=${page}" <#if page == questions.data.index>class="active"</#if>> ${page} </a>
        </#list>
        ...
        <#if questions.data.hasNextPage>
            <a href="${applicationContext}/question/category/${tag.name()}?index=${questions.data.maxPage}">末页</a>
        </#if>
    </div>
</div>
