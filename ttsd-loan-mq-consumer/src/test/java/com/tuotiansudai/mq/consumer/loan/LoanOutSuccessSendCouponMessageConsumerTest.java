...
public class LoanOutSuccessSendCouponMessageConsumerTest {
    @Autowired
    @Qualifier("loanOutSuccessSendCouponMessageConsumer")
    ...
    public void shouldConsumeIsOk() throws JsonProcessingException {
        ...
        when(payWrapperClient.sendRedEnvelopeAfterLoanOut(anyLong())).thenReturn(new BaseDto<>(true));
        when(payWrapperClient.assignInvestAchievementUserCoupon(anyLong())).thenReturn(new BaseDto<>(true));
