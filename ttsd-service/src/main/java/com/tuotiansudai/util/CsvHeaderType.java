...
public enum CsvHeaderType {
    ...
    InterestCouponsHeader("名称,利率(%),总投资金额(元),活动期限,有效天数,发放对象,预计发放数量(张),短信提醒,实际发放张数(张),已使用(张),可投标的,使用条件,应发放总收益(元),已发放收益(元)", "加息券管理"),
    RedEnvelopesHeader("名称, 金额(元), 总投资金额(元), 活动期限, 有效天数, 已发放(张), 已使用(张), 发放对象, 可投标的, 起投金额, 应发放总收益(元), 已发放收益(元), 是否共用", ""),
    BirthdayCouponsHeader("名称, 翻倍倍数, 总投资金额(元), 活动期限, 发放对象, 可投标的, 已使用(张), 应发放总收益(元), 已发放收益(元), 推送提醒, 操作", "生日月活动管理"),
    PointPrizeHeader("奖品, 中奖人数, 状态", "财逗奖品管理");
