...
@ActiveProfiles("test")
public class AdvanceInvestRepaySuccessService {
    ...
    public void investRepaySuccess(){
        ...
        when(investRepayMapper.findById(anyLong())).thenReturn(mockInvestRepayModel(1, 1, RepayStatus.REPAYING));
        ...
        when(investRepayMapper.findByInvestIdAndPeriodAsc(anyLong())).thenReturn(Lists.newArrayList(mockInvestRepayModel(2, 2, RepayStatus.REPAYING), mockInvestRepayModel(3, 3, RepayStatus.REPAYING)));

        investRepaySuccessService.processAdvancedInvestRepaySuccess(mockBankLoanCallbackMessage());
        ...
        verify(investRepayMapper, times(3)).update(investRepayModelCaptor.capture());
        ...
        assertThat(investRepayModelCaptor.getAllValues().size(), is(3));
        assertThat(investRepayModelCaptor.getAllValues().get(0).getStatus(), is(RepayStatus.COMPLETE));
        assertThat(investRepayModelCaptor.getAllValues().get(1).getStatus(), is(RepayStatus.COMPLETE));
        assertThat(investRepayModelCaptor.getAllValues().get(2).getStatus(), is(RepayStatus.COMPLETE));
        assertThat(investRepayModelCaptor.getAllValues().get(0).getRepayAmount(), is(1009L));
    ...
    public void investRepayFail(){
        when(investMapper.findById(anyLong())).thenReturn(mockInvestModel());
        when(investRepayMapper.findById(anyLong())).thenReturn(mockInvestRepayModel(1, 1, RepayStatus.COMPLETE));
        investRepaySuccessService.processAdvancedInvestRepaySuccess(mockBankLoanCallbackMessage());
    ...
    private InvestRepayModel mockInvestRepayModel(long id, int period, RepayStatus status){
        return new InvestRepayModel(id, 1, period, period == 3 ? 1000 : 0, 10, 1, DateTime.now().plusDays(1).toDate(), status);
    }

    private LoanRepayModel mockLoanRepayModel(){
        LoanRepayModel loanRepayModel = new LoanRepayModel(1, 1, 1, 1000, 10, DateTime.now().plusDays(1).toDate(), RepayStatus.COMPLETE);
