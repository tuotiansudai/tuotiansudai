...
public class ProductManageController {
    ...
    @RequestMapping(value = "/{goodsId:^\\d+$}/detail", method = RequestMethod.GET)
    public ModelAndView find(@PathVariable long goodsId,
                             @RequestParam(value = "index", required = false, defaultValue = "1") int index,
                             @RequestParam(value = "pageSize", required = false, defaultValue = "10") int pageSize) {
        ModelAndView modelAndView = new ModelAndView();
        modelAndView.addObject("orders", productService.findProductOrderList(goodsId, null, index, pageSize));
        modelAndView.addObject("product", productService.findById(goodsId));
        long ordersCount = productService.findProductOrderCount(goodsId);
