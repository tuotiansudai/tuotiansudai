...
@Service
public class ActivityConsoleCelebrationHeroRankingService {

    static Logger logger = Logger.getLogger(ActivityConsoleCelebrationHeroRankingService.class);
    ...
    public List<NewmanTyrantView> obtainHero(Date tradingTime) {
        ...
        List<NewmanTyrantView> tyrantViews = investCelebrationHeroRankingMapper.findCelebrationHeroRankingByTradingTime(tradingTime, celebrationHeroRankingActivityPeriod.get(0), celebrationHeroRankingActivityPeriod.get(1));
    ...
    private List<Date> obtainActivityDays(Date tradingTime) {
        ...
        Date activityBeginTime = DateTime.parse(celebrationHeroRankingActivityPeriod.get(0), DateTimeFormat.forPattern("yyyy-MM-dd HH:mm:ss")).toDate();
        Date activityEndTime = DateTime.parse(celebrationHeroRankingActivityPeriod.get(1), DateTimeFormat.forPattern("yyyy-MM-dd HH:mm:ss")).toDate();
    ...
    public List<NewmanTyrantHistoryView> obtainNewmanTyrantHistoryRanking(Date tradingTime) {
        ...
        for (Date currentDate : dateList) {
            List<NewmanTyrantView> newmanViews = obtainHero(currentDate);
            long avgTyrantInvestAmount = newmanViews.stream().mapToLong(heroRankingView -> heroRankingView.getSumAmount()).sum();
            avgTyrantInvestAmount = new BigDecimal(avgTyrantInvestAmount).divide(new BigDecimal(newmanViews.size() == 0 ? 1 : newmanViews.size()), 0, RoundingMode.DOWN).longValue();
