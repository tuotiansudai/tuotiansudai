<div class="store-footer">
    <div class="wp clearfix footer-list">
        <dl class="list-normal">
            <dt>会员中心</dt>
            <dd><a href="/membership">我的会员</a></dd>
            <dd><a href="/membership/structure">成长体系</a></dd>
            <dd><a href="/membership/privilege">会员特权</a></dd>
        </dl>
        <dl class="list-normal">
            <dt>积分消费</dt>
            <dd><a href="${pointsystemServer}/pointsystem">积分商城</a></dd>
            <!-- <dd><a href="/membership/lottery">积分抽奖</a></dd> -->
        </dl>
        <dl class="list-normal">
            <dt>赚取积分</dt>
            <dd><a href="/loan-list">我要投资</a></dd>
            <dd><a href="${pointsystemServer}/pointsystem/task">积分任务</a></dd>
        </dl>
        <dl class="list-contact">
            <dt>联系我们</dt>
            <dd><a href="javascript:void(0)">客服电话：<span>400-169-1188</span> ( 服务时间：9:00-20:00 )</a></dd>
            <dd><a href="javascript:void(0)">商务合作：<span>010-47167320</span></a></dd>
        </dl>
        <div class="images-app">
            <i class="img-app-download"></i>
            <p class="tc">APP下载</p>
        </div>
    </div>
    <div class="copyright-info">
        京ICP备15035567号   拓天伟业（北京）金融信息服务有限公司  版权所有 
    </div>
</div>