...
<div class="register-container page-width register-account">
    ...
    <div class="step-account-head">
        <span class="info-note">为了保障您的账户及资金安全，根据相关法律法规，我们会对您进行实名认证。</span>
        ...
        <form class="register-account-form" action="/register/account" id="registerAccountForm" method="post">
            <ul class="reg-list tl register-step-two">
                <li>
                    ...
                    <input type="text" name="userName" placeholder="请输入您的真实姓名" maxlength="17" class="user-name" value="${(originalFormData.userName)!}"/>
                </li>
                <li><label class="title">身份证号</label>
                    <input type="text" name="identityNumber" placeholder="请输入您的身份证号码" class="identity-number" maxlength="18" value="${(originalFormData.identityNumber)!}"/>
                </li>
            </ul>
