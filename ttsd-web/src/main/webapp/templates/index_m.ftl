                ...
                <li style="opacity: 0;" class="">
                    <a href="${banner.url}" target="_blank"
                       <#if banner.url == 'http://www.iqiyi.com/w_19rt7ygfmh.html#vfrm=8-8-0-1'>rel="nofollow"</#if>>
                        <img src="${commonStaticServer}${banner.webImageUrl}"
                             data-app-img="${commonStaticServer}${banner.appImageUrl}" alt="${banner.title}">
                    </a>
...
<#--优选债权-->
            ...
            <div class="target-category-box you" data-url="/m/loan/${loan.id?c}">
                ...
                <#if loan.status!= 'PREHEAT'>
                    <div class="table-row progress-column">
                        <div class="progress-bar">
                            <div class="process-percent ">
                                <div class="percent <#if ['RECHECK', 'REPAYING', 'OVERDUE', 'COMPLETE']?seq_contains(loan.status)>colorChange</#if>"
                                     style="width:${loan.progress}%">
                                </div>
                            </div>
                        </div>
                        <span class="p-title">剩余金额：<i><em
                                class="money"><@amount>${loan.availableInvestAmountCent?c}</@amount></em>元</i></span>
                    </div>
                <#else>
                    <div class="table-row progress-column">
                        <div class="progress-bar">
                ...
                <i class="second_show"></i>秒后开标
                            ...
                            <#else>
                                <span style="color: #FF473C"> ${(loan.fundraisingStartTime?string("yyyy-MM-dd HH时mm分"))!}
                                    开标</span>
                ...
                <ul class="loan-info  clearfix">
                    ...
                    <li>
                        <#if loan.transferStatus=='TRANSFERRING'>
                            <a data-url="/transfer/${loan.transferApplicationId}"
                               class="btn-invest btn-normal goToTranDetail">立即购买</a>
                        <#else>
                            <i class="loan-status icon-transferred"></i>
                        </#if>
                    </li>
                </ul>
                <div class="transfer-price">转让价格：<em
                        class="money"><@percentInteger>${loan.transferAmount}</@percentInteger>
                <@percentFraction>${loan.transferAmount}</@percentFraction></em>元/<em
                        class="money"><@percentInteger>${loan.investAmount}</@percentInteger>
    ...
    <div class="company-info">
        客服电话：400-169-1188（服务时间：工作日 9:00-20:00）<br/>
