<#import "macro/home.ftl" as global>
<@global.main pageCss="${css.index}" pageJavascript="${js.index}" activeNav="首页" activeLeftNav="">
<div class="main-advantage clear-blank-m">
    <dl>
        <dd>
            <img src="${staticServer}/images/icons/hs01.png" alt="超高收益 最低门槛" >
            <span class="clearfix">
                <b class="clearfix">超高收益 最低门槛</b>
                最高46倍活期存款收益 <br/>
                    最低投资门槛1元
            </span>
        </dd>
        <dd>
            <img src="${staticServer}/images/icons/hs02.png" alt="三方托管 放心理财">
            <span class="clearfix">
                <b class="clearfix">三方托管 放心理财</b>
                第三方银行托管 <br/>
                第三方支付
            </span>
        </dd>
        <dd>
            <img src="${staticServer}/images/icons/hs03.png" alt="实力雄厚 安全保障">
            <span class="clearfix">
                <b class="clearfix">实力雄厚 安全保障</b>
                上市企业投资 <br/>
                稳定安全债权来源
            </span>
        </dd>
    </dl>
</div>

<div class="home-content bg-w clearfix">
    <section class="product-box-list fl">
        <div class="product-box-inner">
            <div class="product-box tc new-standard">
                <i class="hot-new"></i>
                <div class="pad-m">
                    <h2 class="pr-title">活期理财</h2>
                    <div class="pr-square tc">
                        <div class="pr-square-in">
                            <em><b>14</b>+1%</em>
                            <i class="clearfix">年化收益</i>
                        </div>
                    </div>
                    <dl class="pr-info">
                        <dd><i>3</i>个月<br/><span>项目期限</span></dd>
                        <dd>500万<br/><span>项目金额</span></dd>
                    </dl>
                    <div class="project-schedule clear-blank">
                        <div class="p-title">
                            <span class="fl">项目进度</span>
                            <span class="point fr">50%</span>
                        </div>
                        <div class="process-percent">
                            <div class="percent" style="width:50%"></div>
                        </div>
                    </div>
                </div>
                <a href="#" class="btn-normal">立即投资</a>
            </div>
            <div class="product-box tc">
                <i class="hot-new"></i>
                <div class="pad-m">
                    <h2 class="pr-title">活期理财</h2>
                    <div class="pr-square tc">
                        <div class="pr-square-in">
                            <em><b>14</b>+1%</em>
                            <i class="clearfix">年化收益</i>
                        </div>
                    </div>
                    <dl class="pr-info">
                        <dd><i>3</i>个月<br/><span>项目期限</span></dd>
                        <dd>500万<br/><span>项目金额</span></dd>
                    </dl>
                    <div class="project-schedule clear-blank">
                        <div class="p-title">
                            <span class="fl">项目进度</span>
                            <span class="point fr">50%</span>
                        </div>
                        <div class="process-percent">
                            <div class="percent" style="width:50%"></div>
                        </div>
                    </div>
                </div>
                <a href="#" class="btn-normal">立即投资</a>
            </div>
            <div class="product-box tc">
                <i class="hot-new"></i>
                <div class="pad-m">
                    <h2 class="pr-title">活期理财</h2>
                    <div class="pr-square tc">
                        <div class="pr-square-in">
                            <em><b>14</b>+1%</em>
                            <i class="clearfix">年化收益</i>
                        </div>
                    </div>
                    <dl class="pr-info">
                        <dd><i>3</i>个月<br/><span>项目期限</span></dd>
                        <dd>500万<br/><span>项目金额</span></dd>
                    </dl>
                    <div class="project-schedule clear-blank">
                        <div class="p-title">
                            <span class="fl">项目进度</span>
                            <span class="point fr">50%</span>
                        </div>
                        <div class="process-percent">
                            <div class="percent" style="width:50%"></div>
                        </div>
                    </div>
                </div>
                <a href="#" class="btn-normal">立即投资</a>
            </div>
            <div class="product-box tc">
                <i class="hot-new"></i>
                <div class="pad-m">
                    <h2 class="pr-title">活期理财</h2>
                    <div class="pr-square tc">
                        <div class="pr-square-in">
                            <em><b>14</b>+1%</em>
                            <i class="clearfix">年化收益</i>
                        </div>
                    </div>
                    <dl class="pr-info">
                        <dd><i>3</i>个月<br/><span>项目期限</span></dd>
                        <dd>500万<br/><span>项目金额</span></dd>
                    </dl>
                    <div class="project-schedule clear-blank">
                        <div class="p-title">
                            <span class="fl">项目进度</span>
                            <span class="point fr">50%</span>
                        </div>
                        <div class="process-percent">
                            <div class="percent" style="width:50%"></div>
                        </div>
                    </div>
                </div>
                <a href="#" class="wait-invest btn-normal">预热中</a>
            </div>
            <div class="product-box tc">
                <i class="hot-new"></i>
                <div class="pad-m">
                    <h2 class="pr-title">活期理财</h2>
                    <div class="pr-square tc">
                        <div class="pr-square-in">
                            <em><b>14</b>+1%</em>
                            <i class="clearfix">年化收益</i>
                        </div>
                    </div>
                    <dl class="pr-info">
                        <dd><i>3</i>个月<br/><span>项目期限</span></dd>
                        <dd>500万<br/><span>项目金额</span></dd>
                    </dl>
                    <div class="project-schedule clear-blank">
                        <div class="p-title">
                            <span class="fl">项目进度</span>
                            <span class="point fr">50%</span>
                        </div>
                        <div class="process-percent">
                            <div class="percent" style="width:50%"></div>
                        </div>
                    </div>
                </div>
                <a href="#" class="btn-normal wait-invest">预热中</a>
            </div>
            <div class="product-box tc">
                <i class="hot-new"></i>
                <div class="pad-m">
                    <h2 class="pr-title">活期理财</h2>
                    <div class="pr-square tc">
                        <div class="pr-square-in">
                            <em><b>14</b>+1%</em>
                            <i class="clearfix">年化收益</i>
                        </div>
                    </div>
                    <dl class="pr-info">
                        <dd><i>3</i>个月<br/><span>项目期限</span></dd>
                        <dd>500万<br/><span>项目金额</span></dd>
                    </dl>
                    <div class="project-schedule clear-blank">
                        <div class="p-title">
                            <span class="fl">项目进度</span>
                            <span class="point fr">50%</span>
                        </div>
                        <div class="process-percent">
                            <div class="percent" style="width:50%"></div>
                        </div>
                    </div>
                </div>
                <button type="button" disabled class="btn-normal">已售罄</button>
            </div>
        </div>
        <div class="tr clear-blank">
            <a href="/loan-list">更多>></a>
        </div>
    </section>
    <aside class="home-ad fr">
        <img src="${staticServer}/images/sign/ad-h.jpg">
    </aside>
</div>
</@global.main>