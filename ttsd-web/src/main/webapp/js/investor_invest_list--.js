require(['jquery', 'template', 'mustache', 'text!/tpl/investor-invest-repay-table.mustache', 'text!/tpl/investor-invest-experience-repay-table.mustache', 'moment', 'pagination', 'layerWrapper', 'daterangepicker', 'jquery.ajax.extension'], function ($, tpl, Mustache, investRepayTemplate, investExperienceRepayTemplate, moment, pagination, layer) {
    ...
    $('body').on('click', '.show-invest-repay', function (event) {
        ...
        $.ajax({
            ...
            if (data.status) {
                ...
                console.log(data);
                var html = data.loanId == 1 ? Mustache.render(investExperienceRepayTemplate, data) : Mustache.render(investRepayTemplate, data);
