...
public class BindBankCardController {
    ...
    public ModelAndView bindBankCard(HttpServletRequest request, @PathVariable(value = "source") Source source) {
        BankAsyncMessage bankAsyncData = bankBindCardService.bind(LoginUserInfo.getLoginName(), source, RequestIPParser.parse(request), null, LoginUserInfo.isRole(Role.INVESTOR));
    ...
    public ModelAndView unbindBankCard(HttpServletRequest request, @PathVariable(value = "source") Source source) {
        BankAsyncMessage bankAsyncData = bankBindCardService.unbind(LoginUserInfo.getLoginName(), source, RequestIPParser.parse(request), null, LoginUserInfo.isRole(Role.INVESTOR));
        return new ModelAndView("/pay", "pay", bankAsyncData);
    }
